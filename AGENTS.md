...
| 「取不到数」「被限流了」「今天怎么什么都拿不到」 | 先 `python tools/astock.py budget`（看请求计数是否失控），再 `cooldown`（看是不是在冷却期）。**计数无上限，但不要靠重试解决，重试是这个问题的成因** |
...
**也不要为了"把数据凑齐"而反复重跑取数。** 系统保留每日请求计数（无上限；
一次正常复盘约 30–50 个），用于发现重复取数。
