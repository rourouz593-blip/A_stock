#!/usr/bin/env python3
"""从可发现 agent packages 与 AGENTS.md 生成各 harness 的适配文件。
...
所以本项目的做法是：
    ...
    agents/*/AGENT.md + SKILL.md + AGENTS.md  ← harness-neutral source
              ↓ sync_harness.py
...
生成器是唯一的 harness-specific 边界；业务角色与方法不在适配文件中复制。
"""
...
GEN_MARK = "<!-- 本文件由 tools/sync_harness.py 生成，不要手改；改 agent package 或 AGENTS.md 后重新生成 -->"
...
def agents() -> list[dict]:
    from core.agent_registry import agent_files, read_meta, skill_files

    out = []
    for p in agent_files():
        fm = read_meta(p)
        fm["_path"] = p.relative_to(REPO_ROOT).as_posix() if p.is_relative_to(REPO_ROOT) else str(p)
        fm["_skills"] = [s.relative_to(REPO_ROOT).as_posix()
                         if s.is_relative_to(REPO_ROOT) else str(s)
                         for s in skill_files(p, fm)]
        out.append(fm)
...
def build() -> dict[str, str]:
    ...
    for a in ags:
        files[f".claude/agents/{a['name']}.md"] = (
            ...
            f"- 结构：`{a.get('schema')}`\n"
            f"- 技能：{'、'.join(f'`{s}`' for s in a['_skills']) or '（无）'}\n"
